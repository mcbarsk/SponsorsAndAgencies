...
import dk.ms.SponsorsAndAgenciesModel.Reader.AvgReport;
...
public class WorldGUIAvgReport extends JPanel {
	private static final long serialVersionUID = 1L;
	private final JPanel panel = new JPanel();
	private Settings settings;
	private JTable table;
	private AvgReport avg;
	private boolean init = true;
	
	public WorldGUIAvgReport(ArrayList<WorldIDType> worldIDList, WorldIDType selectedWorldID) {
		...
		if (init && selectedWorldID != null){ // use input ID, if it exists.
			avg = new AvgReport(settings, selectedWorldID.getWorldID());
			table.setModel(avg);			// the table is populated
			init = false;
		...
		combo_WorldID.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (init){
					...
					avg = new AvgReport(settings, ((WorldIDType)combo_WorldID.getSelectedItem()).getWorldID());
					table.setModel(avg);
				}
			}
		});
